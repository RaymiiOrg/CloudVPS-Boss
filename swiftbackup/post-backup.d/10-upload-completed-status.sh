...
touch "/etc/swiftbackup/status/${HOSTNAME}/completed"
if [[ $? -ne 0 ]]; then
...
OLD_IFS="${IFS}"
IFS=$'\n'
SWIFTTOUCH=$(swift upload cloudvps-duplicity-backup "/etc/swiftbackup/status/${HOSTNAME}/completed" --object-name "status/${HOSTNAME}/completed" 2>&1 | grep -v -e UserWarning -e pkg_resources)
if [[ $? -ne 0 ]]; then
