...
touch "/etc/swiftbackup/status/${HOSTNAME}/started"
if [[ $? -ne 0 ]]; then
...
OLD_IFS="${IFS}"
IFS=$'\n'
SWIFTTOUCH=$(swift upload cloudvps-duplicity-backup "/etc/swiftbackup/status/${HOSTNAME}/started" --object-name "status/${HOSTNAME}/started" 2>&1 | grep -v -e UserWarning -e pkg_resources)
if [[ $? -ne 0 ]]; then
